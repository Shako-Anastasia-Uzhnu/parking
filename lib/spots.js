...
export function getCategories() {
  return ["Всі", ...new Set(spots.map((spot) => spot.category))]
}
...
export function addSpot(data) {
  const newSpot = {
    ...
    emoji: data.emoji || '🚗',
    category: data.category || 'Легкові',
...
export function updateSpot(id, data) {
  const index = spots.findIndex((spot) => spot.id === Number(id))
